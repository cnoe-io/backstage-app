...
export const ChatHistoryToolbar = ({ isStreaming, onClear }: ChatHistoryToolbarProps) => {
    ...
    return (
        <div>
            <div></div>
            <div className={`${style.container} ${isStreaming ? style.streaming : ''}`}>
                <img className={style.pulse} src={BedrockLogo} />
                <div className={style['pulse-ring']} />
            </div>
            <div className={style.clearButton}>
                <Button onClick={onClear} disabled={disabled} variant='outlined'>Clear Chat</Button>
            </div>
        </div>
    )
}