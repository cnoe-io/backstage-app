...
export function useWellArchitectedReview({
  component,
}: {
  component: string;
}) {
  const wellArchitectedApi = useApi(awsWellArchitectedApiRef);

  const getReview = useCallback(
    async () => {
      return await wellArchitectedApi.getReviewByComponent({
        component,
      });
    },
  ...
  const {
    ...
    error,
    retry,
  } = useAsyncRetry<LensReview | undefined>(async () => {
